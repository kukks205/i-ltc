...
left join (select 
count(distinct dp1.person_id) as screen ,
d.hospcode,d.last_calc 
from (select
max(vn) as last_vn,hospcode,person_id,feel_boring_2_week,feel_depression_2_week,depression_score,dp.last_calc
from
ltc_depression_screen AS dp where dp.house_regist_type_id in (1,3)
group by dp.hospcode,dp.person_id) as d
join ltc_depression_screen dp1 on dp1.vn=d.last_vn 
...
ltc_depression_screen AS dp where dp.house_regist_type_id in (1,3)
group by dp.hospcode,dp.person_id) as d
join ltc_depression_screen dp on dp.vn=d.last_vn 
where dp.feel_boring_2_week='N' and dp.feel_depression_2_week='N' and dp.depression_score<7
